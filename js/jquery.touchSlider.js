 ...
 * @since	201106
 *
 * @param Object	settings	환경변수 오브젝트(环境变量对象)
 *		roll			-	순환 (default true)(循环)
 *		flexible		-	유동 레이아웃 (default true)(流量布局)
 *		resize			-	리사이즈 사용 (default false)(使用调整大小)
 *		view			-	다중 컬럼 (default 1)(多列)
 *		speed			-	애니메이션 속도 (default 150)(动画速度)
 *		range			-	넘김 판정 범위 (default 0.15)(检查范围)
 *		page			-	초기 페이지 (default 1)(初始页)
 *		transition		-	CSS3 transition 사용 (default true)(css3动画使用)
 *		btn_prev		-	prev 버튼 (jQuery Object, default null)(上一页按钮)
 *		btn_next		-	next 버튼 (jQuery Object, default null)(下一页按钮)
 *		paging			-	page 버튼 (jQuery Object, default null)(页面按钮)
 *		sidePage		-	사이드 페이지 사용 (default false)(使用侧页)
 *		initComplete 	-	초기화 콜백(初始化回调)
 *		counter			-	슬라이드 콜백, 카운터(幻灯片回调，计数器)
 *		autoplay		-	자동움직임 관련 옵션 (Object)(自动移动选项)
 *
 * @example
